// Full-screen particle burst + expanding-ring effect for heart taps. Plain
// module (not a component): this is a self-driving canvas animation loop,
// not something that benefits from React re-renders — it manages its own
// requestAnimationFrame loop and stops itself when nothing's left to draw.
let canvas, ctx, dpr;
let particles = [];
let rings = [];
let running = false;
...
function halfRect(side) {
  ...
  return side === 'blue' ? [0, 0, cx, window.innerHeight] : [cx, 0, window.innerWidth - cx, window.innerHeight];
}
...
function spawnRings(x, y, color, side) {
  ...
  for (let i = 0; i < 3; i++) {
    rings.push({ x, y, side, life: -i * 9, maxLife: 65, maxRadius, color });
  }
}
...
export function initTapFX(canvasEl) {
  canvas = canvasEl;
...
export function triggerTapFX(x, y, color, side) {
  if (!canvas) return;
